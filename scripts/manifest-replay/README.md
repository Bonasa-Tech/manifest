...
Only Manifest invocations that include the selected market are replayed. Setup and cleanup instructions owned by wrappers, aggregators, the system program, and token programs are excluded, as are Manifest instructions for other markets in the same routed transaction. An inner invocation's account privileges are reconstructed from the enclosing transaction because RPC inner-instruction records do not contain the narrower CPI privilege flags. The captured currently deployed program provides the control run; `old replay vs captured chain` in the report makes any environmental mismatch visible.
