...
//! the core Red-Black tree operations. The assembly functions are compiled
//! separately using the sbpf SDK and linked via FFI.
//!
//! # Feature Flags
//!
...
//!
//! To use this module:
//! 1. Install the sbpf SDK: `cargo install --git https://github.com/blueshift-gg/sbpf.git`
//! 2. Build the assembly: `cd lib/asm && sbpf build`
//! 3. Link the resulting .so file via build.rs
//!
//! # Safety
//!
...
///
/// These functions are implemented in lib/asm/src/*.s and compiled
/// to sBPF bytecode using the sbpf SDK.
///
/// # Function Signatures (Assembly Convention)
///
/// All functions follow the sBPF calling convention:
/// - r1-r5: Arguments
/// - r0: Return value
/// - r6-r9: Callee-saved
///
...
extern "C" {
    ...
    /// - root: Pointer to root index (may be updated)
    /// - out_current: Output pointer for next current index
    /// - out_parent: Output pointer for next parent index
    ///
    /// # Safety
    /// Caller must ensure parent points to a valid node.
    pub fn remove_fix_asm(
        data: *mut u8,
        current: u32,
        parent: u32,
        root: *mut u32,
...
pub unsafe fn remove_fix_asm_wrapper(
    ...
    let mut out_current: DataIndex = 0;
    let mut out_parent: DataIndex = 0;
    remove_fix_asm(
        data.as_mut_ptr(),
        current_index,
        parent_index,
        root_index as *mut DataIndex,
        &mut out_current as *mut DataIndex,
        &mut out_parent as *mut DataIndex,
    );
    (out_current, out_parent)
}
