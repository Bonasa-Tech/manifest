...
;
; OPTIMIZATIONS:
; - Single NIL constant load at start (saves 2 mov64 instructions)
; - Batched memory reads before writes (better cache behavior)
; - Eliminated redundant pointer recalculations
; - Combined conditional branches where possible
...
rotate_left_asm:
    ...
    ldxw r7, [r6+NODE_RIGHT]            ; r7 = p_index = g->right
    ldxw r9, [r6+NODE_PARENT]           ; r9 = gg_index = g->parent

    ; Calculate p_ptr and load y_index
    add64 r4, r1, r7                    ; r4 = p_ptr = data + p_index
    ldxw r8, [r4+NODE_LEFT]             ; r8 = y_index = p->left

    ; === Batch all writes ===
    ; Update P node (p->parent = gg, p->left = g)
    stxw [r4+NODE_PARENT], r9
    stxw [r4+NODE_LEFT], r2

    ; Update G node (g->parent = p, g->right = y)
    stxw [r6+NODE_PARENT], r7
    stxw [r6+NODE_RIGHT], r8
