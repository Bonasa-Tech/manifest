...
async fn cu_create_market_test() -> anyhow::Result<()> {
    ...
    let create_market_ixs: Vec<Instruction> =
        create_market_instructions(&market_keypair.pubkey(), &base_mint, &quote_mint, &payer).map_err(|e| anyhow::anyhow!("{e:?}"))?;
...
async fn cu_batch_update_with_globals_test() -> anyhow::Result<()> {
    ...
    send_tx_with_retry(
        Rc::clone(&test_fixture.context),
        &create_market_instructions(&market, &base_mint, &quote_mint, &payer).map_err(|e| anyhow::anyhow!("{e:?}"))?[..],
        Some(&payer),
        &[&payer_keypair, &market_keypair],
    )
    .await?;
